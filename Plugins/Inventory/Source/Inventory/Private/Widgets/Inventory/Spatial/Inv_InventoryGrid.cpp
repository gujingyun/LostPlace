...
void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
{
	...
	const FIntPoint StartingCoordinate = CalculateStartingCoordinate(Parameters.TileCoordinats/*鼠标悬浮所在的格子坐标*/, Dimensions, Parameters.TileQuadrant/*格子的象限*/);
...
FInv_SpaceQueryResult UInv_InventoryGrid::CheckHoverPosition(const FIntPoint& Position, const FIntPoint& Dimensions)
{
	...
	// 如果多个索引被同一个项目占据，我们需要查看它们是否都具有相同的左上角索引。
	...
	// 如果是这样，是否只有一个项目挡路？（我们可以交换吗？
	if (OccupiedUpperLeftIndices.Num() == 1) // 位置上的单个项目 - 它对交换/组合有效
	{
...
{
	// 需要将物品的中心设置为鼠标所在的格子的中心，所以需要根据格子的象限来计算起始坐标
...
void UInv_InventoryGrid::AssignHoverItem(UInv_InventoryItem* InventoryItem)
{
	...
	IconBrush.SetResourceObject(Cast<UObject>(ImageFragment->GetIcon()));
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		// 我们应该交换他们的堆栈数量吗？(Room in the clicked slot == 0 && HoveredStackCount < MaxStackSize)
		if (ShouldSwapStackCounts(RoomInClickedSlot, HoveredStackCount, MaxStackSize))
		{
			SwapStackCounts(ClickedStackCount, HoveredStackCount, GridIndex);
			return;
		}
		// 我们应该消耗悬停物品的堆栈吗？ (Room in the clicked slot >= HoveredStackCount)
		...
		// 我们应该填满被点击项目的堆栈吗？ (and not consume the hover item)
...
{
	FSlateBrush Brush;
	Brush.SetResourceObject(Cast<UObject>(ImageFragment->GetIcon()));
...
void UInv_InventoryGrid::ConstructGrid()
{
	...
	for (int32 j = 0; j < Rows; ++j)
	{
		...
		{
			UInv_GridSlot* GridSlot = CreateWidget<UInv_GridSlot>(GetWorld(), GridSlotClass);
